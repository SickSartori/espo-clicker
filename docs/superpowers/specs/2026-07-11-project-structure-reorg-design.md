...
- **C — Logica & UI (de-monolite + unificazione JS)**: `game-logic` → `src/game/*`; `ui-functions` → `src/ui/*`; `modals` → `src/ui/modals/*`; `save-db`/`i18n`/`asset-manager` → `src/state`+`src/lib`; **`script.js` → `src/app/` per ultimo**. È il filone più grosso: a sotto-fette per file/dominio. **Tranche C-thin FATTA 2026-07-12** (i 6 file infrastrutturali — backend-config, version-config, error-handler, save-db, i18n, asset-manager — sono moduli `src/{lib,state,app}` con install da main.ts; guard F8 in gamestate.js; bundle a 11 voci, 213 KB; restano i monoliti: game-logic, ui-functions, modals, script).
...
- **E — Asset + mappa**: **`ARCHITECTURE.md` FATTO 2026-07-12** (mappa progetto per nuovi collaboratori: due mondi legacy/V3, boot, stato, build/deploy, "dove metto le mani per X", trappole). Rename `assets/` → img/audio/video **SALTATO di proposito**: `assets/sounds|video` sono chiavi del bucket R2 + whitelist in `php/r2-sign.php` → rinominarle romperebbe le URL firmate in prod (non testabile in locale); "image/sounds/video" sono gia nomi chiari → churn su 23+ punti CSS/PHP per zero leggibilita.
