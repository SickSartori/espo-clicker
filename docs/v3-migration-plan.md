...
> **Fase 6 — fetta 4 FATTA** (2026-07-05, chiude F6): eventi → `EspoV3.events` (`src/game/events.ts`, ctor Decimal iniettato): `goldenBugReward` (bps×30+click×10+10, ×globalMult, lucky×8/frenzy×2+buff click×7/15s), `dailyStreak` (+1 se ieri) + `dailyReward` (secs=600+cap×200, pavimento baseClick×(50cap+50), min 50), `crunchDuration` (60s overclock/30s), `crunchCooldownFromEnd` (300s−30s×rete, floor 60s). Delegate clickGoldenBug + claimDailyBonus + activateCrunchTime. Parità preview: golden bug esatto V3↔legacy sui 3 tipi con bps/click reali; daily scala coi secondi corretti (ratio streak20/1=2.5, 5/1=2.0); crunch timing esatto. Suite: 194 test. **→ FASE 6 COMPLETA.** Resta solo **F7** (single build: drop build.js+dist/, drop CDN break_infinity, self-host gsap/howler/hammer, single bundle Vite, update sw.js+index.php).
