<div id="center-column" class="game-column">
    
    <div class="column-header-center">
        
        <div id="header-left-panel" class="header-stat-box" style="display: none;">
            <span class="header-label"><i class="fa-solid fa-bolt"></i> MOLT.</span>
            <span id="display-career-bonus" class="header-value" style="color: #f1c40f;">x1.00</span>
        </div>

        <div class="header-main-score">
            <span class="header-label-main"><i class="fa-solid fa-bug"></i> BUG RISOLTI</span>
            <div id="score-display">0</div>
        </div>

        <div id="header-right-panel" class="header-stat-box" style="display: none; text-align: right;">
            <span class="header-label"><i class="fa-solid fa-coins"></i> TOKEN</span>
            <span id="prestige-points-display" class="header-value" style="color: #3498db;">0</span>
        </div>

    </div>

    <div id="clicker-section">
    
        <div id="click-feedback-container"></div>
        
        <button id="clicker-btn" title="Risolvi un Bug!">
            <img id="manager-photo-clicked" src="./assets/image/espo-click.webp" alt="Click!" draggable="false">
            <img id="manager-photo-normal" src="./assets/image/espo.webp" alt="Espòòò" draggable="false">
        </button>
        
        <div id="cps-display">BPS: 0.0</div>

        <button id="skill-crunchTime" class="skill-btn" style="display: none;">
            🔥 ESPO FURY 🔥
            <div class="skill-timer">Pronto!</div>
        </button>

        <div id="event-multiplier-display" style="display: none;"></div>
        
        <div id="prestige-bonus-display" class="prestige-info" style="display: none;">Bonus: +0%</div>

        <div id="version-display"></div>
    </div>
</div>