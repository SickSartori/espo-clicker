...
<head>
    ...
    <title>Espò Arcade — Sala Giochi</title>
...
<div id="arcade-fullscreen">
    ...
    <header id="arcade-fs-header">
        ...
        <div class="fs-wallet" id="fs-wallet">
            <div class="wallet-main" title="Bug totali (saldo gioco principale + guadagni arcade)">
                <i class="fa-solid fa-bug wallet-ico"></i>
                <span class="wallet-total" id="fs-bug-total">--</span>
                <span class="wallet-unit">BUG</span>
            </div>
            <div class="wallet-pending" id="fs-bug-pending" title="Bug guadagnati in questa sessione arcade, da incassare">
                <i class="fa-solid fa-coins wallet-ico"></i>
                <span id="fs-pending-val">+0</span>
            </div>
        </div>

        <button id="arcade-fs-close" title="Chiudi (torna al gioco)" onclick="window.close(); setTimeout(()=>{ if(!window.closed) window.location.href='index.php'; }, 100);">
    ...
    <div id="arcade-virtual-pad">
        <div class="vp-section">
            ...
            <div class="vp-dpad">
                <button class="vp-btn vp-up"    data-key="ArrowUp"    title="Su (W)">▲</button>
                <button class="vp-btn vp-left"  data-key="ArrowLeft"  title="Sinistra (A)">◀</button>
                <button class="vp-btn vp-down"  data-key="ArrowDown"  title="Giù (S)">▼</button>
                <button class="vp-btn vp-right" data-key="ArrowRight" title="Destra (D)">▶</button>
            </div>
        </div>

        <div class="vp-section">
            <div class="vp-label">AZIONI</div>
            <div class="vp-actions">
                <button class="vp-action-btn cyan"   data-key="Space" title="Spara / Salta (Spazio)">FIRE</button>
                <button class="vp-action-btn yellow" data-key="KeyX"  title="Speciale (X / F)">X</button>
                <button class="vp-action-btn"        data-key="Enter" title="Start / Pausa">START</button>
            </div>
        </div>
    </div>
</div>
...
(function () {
    ...
    if (root) root.innerHTML =
        ...
        + '<div style="font-family:\'Press Start 2P\',monospace;font-size:1rem;color:#fff;letter-spacing:2px;">SALA GIOCHI RISERVATA</div>'
        + '<div style="max-width:440px;font-size:1.15rem;color:#8a9aaa;line-height:1.4;">Devi accedere a <b style="color:#00d9ff">Espò Clicker</b> per entrare in Sala Giochi.</div>'
        + '<button onclick="window.location.href=\'index.php\'" style="margin-top:6px;font-family:\'Press Start 2P\',monospace;font-size:0.7rem;letter-spacing:2px;color:#fff;background:linear-gradient(180deg,#3498db,#2872a8);border:2px solid #1a3a5a;border-radius:6px;padding:14px 28px;cursor:pointer;">VAI AL GIOCO</button>'
        + '</div>';
})();
</script>
