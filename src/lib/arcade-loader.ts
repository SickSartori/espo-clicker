...
//
// Migrato da js/arcade-loader.js (IIFE classic script, caricato in DOPPIO —
// dentro il bundle legacy E come tag standalone in index.php, che sovrascriveva
// la copia nel bundle) a modulo ESM — kill-legacy periferici. Ora è l'unica
...
function loadCSS(href: string) {
  return new Promise<void>(function (resolve) {
    ...
    for (let i = 0; i < all.length; i++) {
      if ((all[i] as HTMLLinkElement).href.indexOf(href.split('?')[0]!) !== -1) {
        return resolve();
      }
    }
...
function loadScript(src: string) {
  return new Promise<void>(function (resolve, reject) {
    ...
    for (let i = 0; i < all.length; i++) {
      if ((all[i] as HTMLScriptElement).src.indexOf(filename) !== -1) {
        return resolve();
      }
    }
