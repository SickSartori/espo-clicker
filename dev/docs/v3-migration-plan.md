...
> **Fase 5 — fetta 4 FATTA** (2026-07-05): predicati di progressione → `EspoV3.rules` (src/ui/rules/progression.ts: disponibilità upgrade click/auto/prestigio, achievement riscattabili, visibilità tab Promozione/Quantum, somma visualBps — big number via stringa, 15 test). Delegati checkTabNotifications, updateTabsVisibility, checkOverlayNotifications, calculateVisualBPS; al legacy restano mapping dati, costi scalati (formule F6) e DOM. Parità preview: esiti DOM identici V3↔legacy, visualBps identico. Suite: 133 test. La logica migrabile di ui-functions è quasi esaurita: il resto è DOM puro (va via in F7) o formule di game-logic → prossimo passo naturale = F6 a fette (economia: costi team, click value, CPS, soglia prestigio).
> **Fase 6 — fetta 1 FATTA** (2026-07-05): formule economiche → `EspoV3.economy` (src/game/economy.ts): softcap bonus, prestigeBonus, computeBps, clickValue/raw, costo upgrade prestigio (1.5^lvl, sconto quantico, toPrecision/floor), soglia promozione (50M×3^resets). PATTERN CHIAVE: il costruttore Decimal è INIETTATO — a runtime il wrapper passa `window.Decimal` (break_infinity della pagina) → risultati bit-identici al legacy per costruzione; nei test si inietta break_eternity. L'ordine di add/mul replica il legacy alla lettera (l'addizione float non è associativa = contratto di parità). Delegate 6 funzioni in game-logic.js. Harness preview: parità ESATTA (stringhe identiche) su tutte + 13 costi upgrade contro stato reale. Suite: 152 test.
> **Fase 6 — fetta 2 FATTA** (2026-07-05): costi team → `EspoV3.economy` — `teamBulkCost` (serie geometrica, sconto Outsourcing -5%/liv cap -25%, floor/max), `maxAffordableTeams` (formula log ln + raffinamento discendente <10000), `milestoneReached` (soglie fisse + tier 250 oltre 1000). Helper `_teamCostInput` nel legacy mappa r (da costScalingBase/Reduction) e outsourcing. Delegate calculateBulkCost/calculateMaxAffordable. Suite: ~166 test.
> **Fase 6 — fetta 3 FATTA** (2026-07-05): prestigio/formattazione → `EspoV3.prestige` (`src/game/prestige.ts`, ctor Decimal iniettato): `prestigeGained` (cap anti-grind 4×soglia, sqrt/250k), `applyTokenDuplicator` (+20% floored, formula unica dei 2 call-site), `prestigeStartingBugs` (Paracadute 2k/liv + Fast Start 1M), `prestigeTeamCarryover` (Keep Teams min(5+deadline,prev) sui 3 base → Eredità → Accelerazione+1/FastStart+5), `formatQbitsEarned` (1 + sqrt(token/10k)). Delegate calculatePrestigeGained + executePrestige (bug iniziali, carryover team, token dup ×2) + executeFormattingSequence (qBits). Parità preview: calculatePrestigeGained esatto V3↔legacy; moduli puri verificati numericamente (gained a soglia 10315, carryover keep+eredità+accel = 9). Suite: 179 test.
> **Fase 6 — fetta 4 FATTA** (2026-07-05, chiude F6): eventi → `EspoV3.events` (`src/game/events.ts`, ctor Decimal iniettato): `goldenBugReward` (bps×30+click×10+10, ×globalMult, lucky×8/frenzy×2+buff click×7/15s), `dailyStreak` (+1 se ieri) + `dailyReward` (secs=600+cap×200, pavimento baseClick×(50cap+50), min 50), `crunchDuration` (60s overclock/30s), `crunchCooldownFromEnd` (300s−30s×rete, floor 60s). Delegate clickGoldenBug + claimDailyBonus + activateCrunchTime. Parità preview: golden bug esatto V3↔legacy sui 3 tipi con bps/click reali; daily scala coi secondi corretti (ratio streak20/1=2.5, 5/1=2.0); crunch timing esatto. Suite: 194 test. **→ FASE 6 COMPLETA.**
> **Fase 7 "single build" FATTA** (2026-07-09) — **MIGRAZIONE CONCLUSA**. Packaging, zero cambiamento di comportamento (E2E parità 36/36 + smoke/integrazione/cheatboard 9/9 verdi identici a ogni fetta). (1) `break_infinity` self-hostato via npm e caricato come primo script (`dist/break_infinity.min.js`), rimossa la CDN → `window.Decimal` resta break_infinity (verificato: no `.layer`, ha `.mantissa`). (2) `build.js` ELIMINATO: un plugin Vite (`scripts/vite-plugin-legacy.ts`) replica la concatenazione legacy (JS_FILES in ordine + CSS + copia vendor break_eternity/break_infinity) nel hook closeBundle → `npm run build`=`vite build` produce entrambi i bundle. (3) Pulizia riferimenti a build.js (workflow FTP, index.php, README). Path output INVARIATI (dist/ + dist-v3/). gsap/howler/hammer restano su CDN (fuori scope, migrazione hosting). Versioning NON toccato (release process).
> **Fase 8 FATTA** (2026-07-10, chiude la migrazione): rimossi i fallback legacy morti dietro le deleghe `EspoV3?.x ?? legacy` in tutti e 6 i file (save-db, i18n, asset-manager, script, ui-functions, game-logic) a fette F8/0→F8/5c. `EspoV3` è ora un requisito **hard** (guard di boot in `js/save-db.js` che segnala i moduli mancanti). `parity.spec.ts` ritirata (progettata per scadere qui) e sostituita da un test in `integration.spec.ts` che copre gli stessi percorsi col solo ramo V3 attivo. Bundle legacy 304→292 KB. Gate finale: 194 unit + E2E 9/9 + typecheck verdi. **MIGRAZIONE STRANGLER CHIUSA.**
> **Fase 6 — fetta 2 FATTA** (2026-07-05): costi team → `EspoV3.economy` (src/game/economy.ts): teamBulkCost (serie geometrica floored + sconto Outsourcing -5%/lvl cap -25%), maxAffordableTeams (formula log ln + refine loop discendente), milestoneReached (soglie + tier 250 oltre 1000). Ctor Decimal iniettato come fetta 1. Delegate calculateBulkCost, calculateMaxAffordable, checkBuildingMilestone (a quest'ultima solo il calcolo `reached`, toast/flash restano legacy). Harness preview: parità ESATTA su 264 costi bulk + 44 max + 8 milestone (11 team × 4 ordini di grandezza) + acquisto reale end-to-end OK. Nota: maxAffordable replica la sottostima ±1 nota del legacy (formula log floor + refine solo discendente) — parità, non fix. Suite: 164 test. Prossime fette F6: prestigio/formattazione (executePrestige), eventi (golden bug, bluescreen, crunch). Fase 2: anti-rollback client delegato a `EspoV3.save.antiRollback` con la gerarchia del server Format>Prestige>Score (prima confrontava solo lifetimeScore — divergenza voluta e verificata: nei casi limite ora client e server decidono uguale); salvage migrazione cloud V1→V2 delegato a `EspoV3.migrations.migrate` (gate resta su version.major, schemaVersion=1 esplicito); i nuovi save scrivono `schemaVersion: 2` (gamestate.js) per rendere autonomo il framework in futuro. Parità verificata in preview (matrice 8/8 anti-rollback incl. big number e divergenze, 4/4 migrazione). Fase 0: `game.modules.js` caricato prima del bundle legacy (dopo la CDN break_infinity) → `EspoV3` garantito al boot legacy, Decimal deterministico (break_infinity; break_eternity solo fallback se CDN giù). Fase 1: `window.SaveDB` delega a `EspoV3.save` con fallback legacy in `js/save-db.js` — verificata parità round-trip e lettura incrociata nei due sensi in preview. Baseline test: 65 verdi. **Resta di F1:** rimozione dell'implementazione legacy in save-db.js dopo un ciclo di deploy verificato su test/prod.
...
## 2. Ordine di caricamento = ordine di dipendenza (in `scripts/vite-plugin-legacy.ts`)
...
| **7** | ✅ FATTA — self-host break_infinity, `build.js`→plugin Vite (single build tool), pulizia | **M** |
...
- `scripts/vite-plugin-legacy.ts` — concatenazione legacy dentro `vite build` (ex `build.js`, rimosso in F7)
