...
- 🚨 **`save_progress` esplode a fine partita** — trovato il 10/09/2026 ispezionando l'account T3tt3. La RPC confronta i punteggi con `p_score::float8 < cur_score::float8`, ma `score` è `TEXT` e il client manda `toFixed(0)`, cioè tutte le cifre. Oltre ~1,8 × 10³⁰⁸ il cast **solleva un errore** (`22003: out of range for type double precision`, verificato in produzione), la RPC va in eccezione, la Edge Function risponde 500 e **i salvataggi cloud si fermano per sempre** per quel giocatore — nessun messaggio utile, solo `[Save✗] status=error`. Oggi il massimo in classifica è 23 cifre, quindi c'è margine, ma è un muro che prima o poi qualcuno prende in pieno. Rimedio: confronto in `numeric` (precisione arbitraria) invece di `float8`, oppure normalizzare in mantissa+esponente. Da fare **prima** della Season 2, quando i numeri ripartono ma i veterani no.
